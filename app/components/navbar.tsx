...
const isPropertiesRoute = (href: string) => {
  if (/^https?:\/\//.test(href)) return false;

  const routePath = href.split("?")[0].replace(/\/$/, "") || "/";
  return routePath === "/propiedades" || routePath.startsWith("/propiedades/");
};
    ...
    <Link
      ...
      onClick={(event) => {
        ...
        if (
          ...
          !event.shiftKey &&
          !event.altKey &&
          (isPropertiesRoute(window.location.pathname) || isPropertiesRoute(href))
        ) {
              ...
              return (
                <div key={link.name} className="relative flex items-center">
                  {hasSubmenu ? (
                    ...
                    <NavigationLink href={link.href} className={desktopLinkClass}>
                      <span>{link.name}</span>
                    </NavigationLink>
                  )}
                </div>
